...
class JustAudioBackground {
  // ---- Musillow fork: notification actions -------------------------------
  //
  // The upstream package hard-codes its notification controls (and always
  // includes a stop button). These hooks let the host app add Like and Shuffle
  // and own what they do, without this package knowing anything about it.

  /// Called when the notification's Like button is pressed.
  static Future<void> Function()? onLike;

  /// Called when the notification's Shuffle button is pressed.
  static Future<void> Function()? onShuffle;

  /// Whether the current track is liked — drives which Like icon is drawn.
  static bool liked = false;

  /// Whether shuffle is on — drives which Shuffle icon is drawn.
  static bool shuffleEnabled = false;

  /// Set by the handler so the app can redraw the notification after its own
  /// state changes (a like toggled from inside the app, say).
  static void Function()? _redraw;

  /// Redraw the notification to match [liked] / [shuffleEnabled].
  static void refreshControls() => _redraw?.call();

  // ---- Musillow fork: Android Auto / media-browser tree -------------------
  //
  // Upstream never implements the browse side of audio_service — only the
  // "now playing" transport controls. These two hooks let the host app supply
  // a browsable tree (Android Auto, Wear, Assistant) without this package
  // knowing anything about playlists, albums, or how a tapped leaf maps to a
  // queue.

  /// Returns the children of [parentMediaId] ("root" for the top level).
  static Future<List<MediaItem>> Function(String parentMediaId)?
      onGetChildren;

  /// Called when the browser (e.g. Android Auto) picks a playable leaf item.
  static Future<void> Function(String mediaId)? onPlayFromMediaId;
...
class _PlayerAudioHandler extends BaseAudioHandler
  ...
  _PlayerAudioHandler() {
    // Musillow fork: let the host redraw the notification when its own Like or
    // Shuffle state changes from inside the app.
    JustAudioBackground._redraw = _broadcastStateIfActive;
  }
  ...
  Future<dynamic> customAction(
    ...
    // Musillow fork: the Like and Shuffle buttons are the host app's to
    // interpret — it owns the star state and the queue order.
    switch (name) {
      case 'like':
        await JustAudioBackground.onLike?.call();
        return null;
      case 'shuffle':
        await JustAudioBackground.onShuffle?.call();
        return null;
    }
    return super.customAction(name, extras);
  }

  // Musillow fork: relay browse/play-from-id calls (Android Auto, etc.) to the
  // host app, which owns the actual library data and queue-building.
  @override
  Future<List<MediaItem>> getChildren(
    String parentMediaId, [
    Map<String, dynamic>? options,
  ]) async =>
      await JustAudioBackground.onGetChildren?.call(parentMediaId) ?? [];
  ...
  Future<void> playFromMediaId(
    ...
    await JustAudioBackground.onPlayFromMediaId?.call(mediaId);
  }
  ...
  void _broadcastState() {
    // Musillow fork: no stop button (it rendered as a bare white square and
    // duplicates the swipe-away gesture); Like and Shuffle bracket the
    // transport controls instead.
    final controls = [
      if (JustAudioBackground.onLike != null)
        MediaControl.custom(
          name: 'like',
          androidIcon: JustAudioBackground.liked
              ? 'drawable/ic_notif_like_filled'
              : 'drawable/ic_notif_like',
          label: 'Like',
        ),
      if (hasPrevious) MediaControl.skipToPrevious,
      if (_playing) MediaControl.pause else MediaControl.play,
      if (hasNext) MediaControl.skipToNext,
      if (JustAudioBackground.onShuffle != null)
        MediaControl.custom(
          name: 'shuffle',
          androidIcon: JustAudioBackground.shuffleEnabled
              ? 'drawable/ic_notif_shuffle_on'
    ...
    playbackState.add(playbackState.nvalue!.copyWith(
      ...
      // Collapsed notifications fit three actions; give them to the transport
      // controls rather than to Like/Shuffle.
